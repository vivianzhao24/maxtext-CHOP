...
def create_orbax_checkpoint_manager(
  ...
  if dataset_type == "tfds" and checkpointing_data :
    item_handlers = {"items": PyTreeCheckpointHandler(use_ocdbt=True, use_zarr3=True), 'iter': TfDataCheckpointHandler()}
  else:
...
def load_state_if_possible(
  ...
  if checkpoint_manager is not None:
    ...
    if latest_step is not None:
      ...
      elif dataset_type == "tfds" and data_iterator is not None:
        return (
            checkpoint_manager.restore(
                latest_step,
                args=ocp.args.Composite(
                    items=ocp.args.PyTreeRestore(item=abstract_unboxed_pre_state, restore_args=restore_args),
                    iter=TfDataCheckpointRestore(data_iterator.local_iterator),
                ),
            ),
            None,
        )  
...
# TODO: add async checkpointing
# TODO: add logging 
class TfDataCheckpointHandler():
    """Orbax CheckpointHandler for TensorFlow NumpyIterator."""
    ...
    def save(
      ...
      item = item or args.item  # Ensure compatibility with older Orbax API
      print("item type", type(item), flush=True)
      ...
      # Save the checkpoint state to the specified directory
      save_path = checkpoint.save(directory / f'process_{jax.process_index()}-of-{jax.process_count()}' / 'ckpt')
     
    def restore(self, directory: epath.Path, item: Optional[tf.data.NumpyIterator] = None, args: Any = None) -> tf.data.NumpyIterator:
        """Restores the given NumpyIterator from the checkpoint in `directory`."""
        item = item or args.item  # Ensure compatibility with older Orbax API
    ...
    def structure(self, directory: epath.Path) -> Any:
        """Defines the structure for the checkpoint (not required for this case)."""
        return None

    def metadata(self, directory: epath.Path) -> Optional[Any]:
        """Retrieve metadata, if necessary (not needed in this case)."""
        return None

    def finalize(self, directory: epath.Path):
        """Called when the checkpointing process is finalized (not used here)."""
        pass

    def close(self):
        """Cleanup resources (not needed here)."""
        pass
...
try:
  ...
  @ocp.args.register_with_handler(TfDataCheckpointHandler, for_save=True)  # pytype:disable=wrong-arg-types
  @dataclasses.dataclass
  class TfDataCheckpointSave(ocp.args.CheckpointArgs):
    item: Any
    step: int # accept the step counter of the orbax checkpoint manager to update tf.data.Checkpoint 

  @ocp.args.register_with_handler(TfDataCheckpointHandler, for_restore=True)  # pytype:disable=wrong-arg-types
  @dataclasses.dataclass
  class TfDataCheckpointRestore(ocp.args.CheckpointArgs):
    item: Any
