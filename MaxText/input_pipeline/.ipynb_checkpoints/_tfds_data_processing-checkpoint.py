...
def get_datasets(
  ...
  """Load a TFDS dataset."""
  # Commented out original builder line from upstream maxtext repo, which loads a dataset by name 
  #ds_builder = tfds.builder(dataset_name)
  
  # Load dataset from directory
  if not dataset_path:
    ds_builder = tfds.builder_from_directory(dataset_path)
  else: # default to c4 dataset
    print("No dataset path is provided, hence defaulted to c4 dataset")
    ds_builder = tfds.builder(dataset_name)
...
def make_tfds_iterator(
  ...
  train_ds = get_datasets(
      ...
      dataset_path=config.dataset_path # include dataset path in order to load dataset from directory. 
  )
