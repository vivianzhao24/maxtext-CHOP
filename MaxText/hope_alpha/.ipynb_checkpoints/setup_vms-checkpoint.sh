...
initialize_single_host(){
    if [[ "$1" == "--help" || "$1" == "-h" ]]; then 
        ...
        gcloud alpha compute tpus tpu-vm ssh $TPU_NAME --command="cd $MAXTEXT_DIR; export PATH='/home/sa_112413943341569390945/.local/bin:$PATH'; bash arcus-maxtext.sh; bash setup.sh"
    fi
...
initialize_multi_host(){
    if [[ "$1" == "--help" || "$1" == "-h" ]]; then 
        ...
        gcloud alpha compute tpus tpu-vm scp /mnt/arcus/lab/shared/llama3.1-whls/ $TPU_NAME:~/ --recurse --worker=all
