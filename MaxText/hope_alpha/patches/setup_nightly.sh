...
# Nightly mode
    if [[ $DEVICE == "gpu" ]]; then
        ...
        echo "Installing jax-nightly, jaxlib-nightly"
        # Install jax-nightly
        pip3 install --no-index --find-links ${HOME}/maxtext_pip_wheels_main/jax_nightly jax

        # Install jaxlib-nightly
        pip3 install --no-index --find-links ${HOME}/maxtext_pip_wheels_main/jax_nightly jaxlib
        ...
        if [[ -n "$LIBTPU_GCS_PATH" ]]; then
            ...
            pip3 install --no-index --find-links ${HOME}/maxtext_pip_wheels_main/jax_nightly libtpu-nightly
        fi
        echo "Installing nightly tensorboard plugin profile"
        #pip3 install tbp-nightly --upgrade
    fi
    echo "Installing nightly tensorboard plugin profile"
    # Note tensorboard nightly was not downloaded, so just commented out believe. If want to use tb nightly need to download the wheels and uncomment the line below or above
    #pip3 install tbp-nightly --upgrade
else
